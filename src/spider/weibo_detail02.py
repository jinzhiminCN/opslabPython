...
def WeiboDetails():
    ...
    for key in keys:
        try:
            ...
            # driver.get('https://m.weibo.cn/detail/' + key.replace("WEIBO#", ""))
            ...
            print(result)
            value['mvalue'] = result['text']
            createtime = str(value['createtime']).replace("最后评论","").strip()
            if not createtime.startswith("2019"):
                createtime ="2019年"+createtime
            value['createtime'] = createtime

            if result['pl'] > 20:
                scoll = 100
                for i in range(result['pl'] / 10):
                    scoll += 100
