...
if __name__ == "__main__":
    ...
    try:
        sconn = pymysql.connect(host="127.0.0.1", user="root", password="wyb2212852",
                                database="fuzz", charset="utf8")

        iconn = pymysql.connect(host="127.0.0.1", user="root", password="wyb2212852",
                                database="dict", charset="utf8")

        selec_cur = sconn.cursor()
        for i in range(28, 401, 1):
            count = selec_cur.execute(select_sql % i)
            print "table->%d:%d" % (i, count)
            for ii in range(1, count, 5000):
                lst = selec_cur.fetchmany(5000)
                logger.info(" %d analyzing ..." % ii)
                ...
                for data in lst:
                    ...
                    counts = data[1]
                    lst_result.append(template % (nick, cn_nick, '', '', en_nick, digit, counts))
